/************************************************************
* Author:Tieying Chu;Benjamin Cha; Samantha Rodarte
* Email: rodartes@oregonstate.edu  chabe@oregonstate.edu  chuti@oregonstate.edu
...
{
	assert(stack != 0);
	if (stack->q1->head == stack->q1->tail) {
		return 1;
	}
	else {
		return 0;
...
TYPE listStackPop(struct Stack* stack)
{
	...
	struct Queue* remove = stack->q1;
	TYPE valueRemoved = listQueueRemoveFront(remove);
